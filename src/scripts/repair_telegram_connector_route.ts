...
export function evaluateRepairGuards(input: {
    task: any; decision: any; projectId: number; taskId: number;
    expectedRevision: number; expectedChannelId: number;
    expectedDecisionId: number; expectedAssetId: number | null; expectedBodySha: string;
    ownerApprovalRef?: string;
}) {
    const { task, decision, projectId, taskId, expectedRevision, expectedChannelId,
        expectedDecisionId, expectedAssetId, expectedBodySha, ownerApprovalRef } = input;
    const bodySha = createHash('sha256').update(task.draft_text || '', 'utf8').digest('hex');
    return {
        ...
        visual_ready: task.handoff_state === 'ready' && (
...
async function main() {
    ...
    const decision = await prisma.artDirectionDecision.findFirst({
        where: { id: expectedDecisionId, project_id: projectId, content_item_id: taskId }
    });
    if (!owner || !task || !decision) throw new Error('Repair target, owner, or art-direction decision not found');

    const prior = await prisma.workflowEvent.findFirst({
    ...
    if (prior) {
        const recorded = prior.after_state as Record<string, any> | null;
    ...
    const qualityReport = (task.quality_report as Record<string, unknown> | null) || {};
    const preview = {
        apply,
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (existing) {
            if (existing.content_item_id !== taskId) throw new Error('[REPAIR_REPLAY_CONFLICT] Key belongs to another task');
        ...
        if (!freshTask || !freshDecision) throw new Error('[REPAIR_CONFLICT] Repair target changed after preflight');
        ...
        const freshQualityReport = (freshTask.quality_report as Record<string, unknown> | null) || {};
        const freshHandoffBundle = (freshQualityReport.handoff_bundle as Record<string, unknown> | null) || null;
        ...
        if (updated.count !== 1) throw new Error('[REPAIR_CONFLICT] Task changed after preflight');
