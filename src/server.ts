...
server.register(mcpOAuthRoutes);
...
const start = async () => {
    try {
        ...
        const startupHealth = backgroundJobsEnabled
            ? await healthService.getDeepHealth().catch((error: any) => ({
                status: 'error',
        ...
        if (backgroundJobsEnabled) {
            ...
            setInterval(async () => {
                try {
                    ...
                    if (processedOperationalTasks > 0) console.log(`[Scheduler] Processed ${processedOperationalTasks} internal operational tasks.`);
                    if (reactivatedDeferred > 0) console.log(`[Scheduler] Reactivated ${reactivatedDeferred} deferred publication tasks.`);
