...
test('organization researcher scope cannot be overridden by tool arguments', () => {
    const body = {
        method: 'tools/call',
        params: { name: 'ba_search_organization_intelligence', arguments: { organizationId: 999, userId: 999, actorId: 'user:999', query: 'test' } }
    };
    const result = scopeRemoteMcpRequest(body, { userId: 2, actorId: 'user:2', organizationId: 7, profile: 'organization_researcher' });
    assert.equal(result.allowed, true);
