...
type RuleSource = 'platform' | 'voice' | 'matrix' | 'global';
type DimensionKey = 'platform_fit' | 'voice_fit' | 'length_fit' | 'rule_fit';
...
type PolicyMatrixConfig = {
    voices?: Record<string, PolicyRuleSet & { description?: string }>;
    platforms?: Record<string, PolicyRuleSet & { description?: string }>;
    matrix?: Record<string, Record<string, PolicyRuleSet & {
        scoring_weights?: Partial<Record<DimensionKey | 'dictionary_fit' | 'llm_quality', number>>;
    }>>;
};
...
type PolicyFinding = {
    ...
    type: 'missing_phrase' | 'forbidden_phrase' | 'min_chars' | 'max_chars' | 'policy_missing';
    message: string;
    matched?: string;
    suggestion?: string;
};
...
class ContentPolicyMatrixService {
    private readonly DEFAULT_YAML = `voices:
  founder:
...
`;
    ...
    parseYaml(rawYaml?: string | null): PolicyMatrixConfig {
    ...
    derivePolicy(rawYaml: string | null | undefined, context: { platform?: string | null; voice?: string | null }) {
        ...
        return {
            ...
            applied_rules: {
                ...
                required_phrases: this.mergeStringArrays(
                    platformRules?.required_phrases,
                    voiceRules?.required_phrases,
                    matrixRules?.required_phrases
                ),
                forbidden_phrases: this.mergeStringArrays(
                    platformRules?.forbidden_phrases,
                    voiceRules?.forbidden_phrases,
                    matrixRules?.forbidden_phrases
                ),
                narrative_rules: this.mergeStringArrays(
                    platformRules?.narrative_rules,
                    voiceRules?.narrative_rules,
                    matrixRules?.narrative_rules
                ),
                preferred_traits: this.mergeStringArrays(
                    platformRules?.preferred_traits,
                    voiceRules?.preferred_traits,
                    matrixRules?.preferred_traits
                )
            }
        };
    }

    validateText(text: string, rawYaml: string | null | undefined, context: { platform?: string | null; voice?: string | null }) {
        ...
        if (!rawYaml?.trim()) {
            return {
                ...
                findings: [{
                    ...
                    dimension: 'rule_fit' as const,
                    type: 'policy_missing' as const,
        ...
        const platformOnlyRequired = this.mergeStringArrays(
            derivedPolicy.platform_rules?.required_phrases,
            derivedPolicy.matrix_rules?.required_phrases
        );
        const voiceOnlyRequired = this.mergeStringArrays(
            derivedPolicy.voice_rules?.required_phrases,
            derivedPolicy.matrix_rules?.required_phrases
        );
        const platformOnlyForbidden = this.mergeStringArrays(
            derivedPolicy.platform_rules?.forbidden_phrases,
            derivedPolicy.matrix_rules?.forbidden_phrases
        );
        const voiceOnlyForbidden = this.mergeStringArrays(
            derivedPolicy.voice_rules?.forbidden_phrases,
            derivedPolicy.matrix_rules?.forbidden_phrases
        );

        for (const phrase of derivedPolicy.applied_rules.required_phrases) {
            if (!haystack.includes(phrase.toLowerCase())) {
                const source: RuleSource = platformOnlyRequired.includes(phrase) && voiceOnlyRequired.includes(phrase)
                    ? 'matrix'
        ...
        for (const phrase of derivedPolicy.applied_rules.forbidden_phrases) {
            if (haystack.includes(phrase.toLowerCase())) {
                const source: RuleSource = platformOnlyForbidden.includes(phrase) && voiceOnlyForbidden.includes(phrase)
                    ? 'matrix'
        ...
        const weightedScore = Math.round(
            dimensions.platform_fit * weights.platform_fit
    ...
    private normalizeRuleMap(value: any): Record<string, PolicyRuleSet & { description?: string }> {
        ...
        return Object.entries(value).reduce((acc, [key, raw]) => {
            ...
            return acc;
        }, {} as Record<string, PolicyRuleSet & { description?: string }>);
    }

    private normalizeMatrix(value: any) {
        if (!value || typeof value !== 'object' || Array.isArray(value)) {
            return {};
        }
        ...
        return Object.entries(value).reduce((platformAcc, [platformKey, rawPlatform]) => {
            ...
            platformAcc[String(platformKey).trim().toLowerCase()] = Object.entries(rawPlatform).reduce((voiceAcc, [voiceKey, rawVoice]) => {
                const normalized = this.normalizeRuleSet(rawVoice) as any;
                normalized.scoring_weights = this.normalizeWeights((rawVoice as any)?.scoring_weights || {});
    ...
    private normalizeRuleSet(raw: any): PolicyRuleSet & { description?: string } {
    ...
    private mergeStringArrays(...groups: Array<string[] | undefined>) {
        return Array.from(new Set(groups.flatMap((group) => group || [])));
    }

    private normalizeWeights(raw: Partial<Record<DimensionKey | 'dictionary_fit' | 'llm_quality', number>>) {
    ...
    private computeDimensions(findings: PolicyFinding[]) {
        const base = {
            platform_fit: 100,
            voice_fit: 100,
            length_fit: 100,
...
export default new ContentPolicyMatrixService();
