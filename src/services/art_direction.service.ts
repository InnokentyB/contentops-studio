...
export type ArtDirectionDecisionType = typeof ART_DIRECTION_DECISIONS[number];
export type VisualMode = 'required' | 'forbidden' | 'auto_assess';
export type AuthenticityClass = 'ACTUAL_EVIDENCE' | 'OWNER_DOCUMENTATION' | 'CONCEPTUAL_EDITORIAL' | 'SIMULATED_DOCUMENTATION';

export interface ArtDirectionDecisionInput {
    decision: ArtDirectionDecisionType;
    source_content_revision: number;
    channel: string;
    placement: string;
    visual_function?: string | null;
    reason: string;
    post_owns?: string | null;
    visual_adds?: string | null;
    loss_without_visual?: string | null;
    authenticity_class?: AuthenticityClass | null;
    evidence_refs?: unknown[];
    visual_format?: string | null;
    dimensions?: { width?: number; height?: number; aspect_ratio?: string } | null;
    required_text?: string[];
    forbidden_text?: string[];
    visible_copy_budget?: number | null;
    prompt?: string | null;
    alt_text?: string | null;
    acceptance_criteria?: string[];
    recent_asset_refs?: unknown[];
}

export interface VisualReadinessInput {
    enabled: boolean;
...
export function defaultVisualMode(channel: string, placement: string): VisualMode {
    ...
    if (normalizedChannel === 'email') return 'forbidden';
...
export function validateArtDirectionDecision(input: ArtDirectionDecisionInput, visualMode: string): ArtDirectionDecisionInput {
    if (!ART_DIRECTION_DECISIONS.includes(input.decision)) throw new Error('[INVALID_VISUAL_DECISION] Unsupported decision');
    ...
    if (!input.reason?.trim()) throw new Error('[VISUAL_REASON_REQUIRED] Decision reason is required');
...
export function calculateVisualReadiness(input: VisualReadinessInput): { ready: boolean; reason: string | null } {
    if (!input.enabled) return { ready: true, reason: null };
    ...
    if (input.visualMode === 'forbidden') return { ready: true, reason: null };
...
function settingEnabled(value?: string | null) {
    return ['1', 'true', 'yes', 'enabled', 'on'].includes((value || '').trim().toLowerCase());
}

export class ArtDirectionService {
    async isEnabled(projectId: number, client: Prisma.TransactionClient | typeof prisma = prisma) {
    ...
    async acceptContentRevision(client: Prisma.TransactionClient, contentItemId: number, actorId: string) {
        ...
        if (!enabled || mode === 'forbidden') return null;
    ...
    async markRevisionStale(client: Prisma.TransactionClient, contentItemId: number) {
        const item = await client.contentItem.findUnique({ where: { id: contentItemId } });
        if (!item || !(await this.isEnabled(item.project_id, client))) return;
    ...
    async getContext(projectId: number, workItemId: number) {
    ...
    async submitDecision(params: {
        ...
        decision: ArtDirectionDecisionInput;
    }) {
        return prisma.$transaction(async (tx) => {
            ...
            if (cached?.after_state) return cached.after_state;
            ...
            if (!workItem?.content_item || workItem.kind !== 'art_direction') throw new Error('Art-direction work item not found');
            ...
            if (workItem.lease_expires_at && workItem.lease_expires_at < new Date()) throw new Error('[LEASE_EXPIRED] Art-direction lease expired');
            const item = workItem.content_item;
            ...
            const stored = await tx.artDirectionDecision.create({
                data: {
                    ...
                    evidence_refs: (validated.evidence_refs || []) as Prisma.InputJsonValue,
                    visual_format: validated.visual_format,
                    dimensions: (validated.dimensions || {}) as Prisma.InputJsonValue,
                    required_text: (validated.required_text || []) as Prisma.InputJsonValue,
                    forbidden_text: (validated.forbidden_text || []) as Prisma.InputJsonValue,
                    visible_copy_budget: validated.visible_copy_budget,
                    prompt: validated.prompt,
                    alt_text: validated.alt_text,
                    acceptance_criteria: (validated.acceptance_criteria || []) as Prisma.InputJsonValue,
                    recent_asset_refs: (validated.recent_asset_refs || []) as Prisma.InputJsonValue,
                    actor_id: params.actorId
                }
            });
            const stateByDecision: Record<ArtDirectionDecisionType, string> = {
            ...
            await tx.workItem.update({ where: { id: workItem.id }, data: { state: 'completed', result_version: version, result_payload: validated as unknown as Prisma.InputJsonValue, lease_token: null, lease_actor_id: null, lease_expires_at: null } });
            ...
            if (validated.decision === 'GENERATE') {
                ...
                await tx.workItem.upsert({
                    where: { dedupe_key: dedupeKey }, update: {}, create: {
                        ...
                        result_payload: { decision_id: stored.id, prompt: stored.prompt, placement: stored.placement } as Prisma.InputJsonValue
                    }
                });
                ...
                await tx.workItem.upsert({
                    where: { dedupe_key: dedupeKey }, update: {}, create: {
                        ...
                        reason_code: validated.decision, missing_resource_refs: (validated.evidence_refs || []) as Prisma.InputJsonValue
                    }
                });
            }
    ...
    async getReadiness(projectId: number, contentItemId: number) {
    ...
    async attachVisualSource(params: { projectId: number; actorId: string; contentItemId: number; fileUrl?: string; fileDataBase64?: string; fileName?: string; mimeType?: string; provenance: Record<string, unknown>; altText?: string }) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            if (!decision || !['SOURCE_REQUIRED', 'MANUAL_ASSET_REQUIRED'].includes(decision.decision)) throw new Error('[SOURCE_NOT_REQUESTED] Active decision does not request a source');
            ...
            await tx.workItem.upsert({ where: { dedupe_key: dedupeKey }, update: {}, create: {
                ...
                result_payload: { asset_id: asset.id, decision_id: decision.id } as Prisma.InputJsonValue
            } });
    ...
    async assertPublicationReady(projectId: number, contentItemId: number) {
        const readiness = await this.getReadiness(projectId, contentItemId);
        if (!readiness.ready) throw new Error(`[VISUAL_GATE_BLOCKED] ${readiness.reason}`);
        return readiness;
    }

    async backfillProject(projectId: number, actorId: string) {
        if (!(await this.isEnabled(projectId))) throw new Error('[ART_DIRECTION_DISABLED] Enable the project pipeline before backfill');
...
export default artDirectionService;
