...
3. Derive tests before code.
   ...
   For messaging and publishing connectors, define a deterministic provider-boundary test for the normalized payload and a separate rendered-output scenario for platform-generated presentation such as captions, reply previews, link cards, quote blocks, and threading. The rendered scenario may require a bounded real-provider check or human UAT; reconstructed payload equality cannot replace it.

   For versioned artifacts, cover the full review lifecycle: approval of the current revision, mutation invalidating that approval, stale-decision rejection, creation of a new review result, and authorized idempotent recovery from a missing or interrupted transition.
...
- If an external platform changes how content is rendered, test transport metadata and platform-generated presentation separately; neither layer proves the other.
- If approval applies to versioned content, treat the revision identifier as part of the decision contract and test invalidation, replay, concurrency, and recovery explicitly.
...
Before delivery, re-read the Review Rules against the finished matrix and confirm that deterministic guarantees, provider availability, rendered platform behavior, revision-bound approval, and human quality judgments have not been collapsed into one test layer.
