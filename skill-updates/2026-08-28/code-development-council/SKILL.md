...
For versioned artifacts, bind every review and approval to one immutable revision. A content mutation must atomically invalidate prior acceptance, reopen review, and require a new decision. Recovery paths must be authorized, idempotent, audited, and validate exact entity and revision identities without rewriting the content being recovered.
...
For messaging and publishing connectors, verify two separate contracts: the normalized payload at the provider boundary, and the destination's rendered semantics such as captions, reply previews, link cards, quote blocks, and threading. Payload equality does not prove that the platform renders the content once and in the intended structure; use a bounded provider-backed check or explicit human UAT for rendered behavior.

Keep provider-boundary tests import-safe. Put payload construction and provider invocation behind an explicit dependency seam so tests can install a mock without initializing unrelated storage, credentials, clients, or application boot logic. A connector contract test should require only the payload contract and the mocked provider.
...
8. For versioned approvals and external connectors, re-check the revision binding, provider-boundary payload, import isolation, and any required rendered-output evidence.
9. In the final answer, report what changed, what passed, and any residual risk.
