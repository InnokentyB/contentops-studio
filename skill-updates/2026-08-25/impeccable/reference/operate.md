...
Map the durable business entity and its state sequence before laying out an operational surface. Arrange the primary controls in that sequence so the operator can see what happened, what is blocked, and what action is next.

- One business event gets one obvious write path. When normalized controls replace legacy controls, remove the duplicate or demote it to read-only history in the same pass.
- A specialized workspace may filter, explain, or accelerate the canonical lifecycle, but it must use the same identifiers, states, and persisted actions as the general workspace.
- Give operators a verified route back to the canonical entity. Avoid parallel dashboards whose status can drift or whose controls write competing versions of the same fact.
- Test loading, empty, stale, permission, partial-failure, and already-completed states against the lifecycle—not only the ideal first action.

Before delivery, trace each visible write control to its canonical persisted event and scan for legacy controls that can still write the same event through a competing path.
