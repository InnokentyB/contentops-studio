...
assert.match(source, /<option value="cancelled">Отменённые<\/option>/)
assert.match(source, /task\.status === 'cancelled'/)
assert.match(source, /isCancelled \? 'Отменено'/)
assert.match(source, /function isTerminalPublicationTask/)
assert.match(source, /const aOverdue = !aTerminal && aTime < now/)
