...
assert.match(source, /selectedTask\?\.text_state === 'accepted'/)
assert.match(source, /selectedTask\?\.accepted_revision === selectedTask\?\.content_revision/)
assert.match(source, /selectedTask\?\.draft_text/)
...
assert.match(source, /statusCounts\.cancelled/)
assert.match(source, /<option value="cancelled">\{copy\.cancelledPlural\}<\/option>/)
assert.match(source, /cancelledPlural: 'Отменённые'/)
assert.match(source, /cancelledPlural: 'Cancelled'/)
assert.match(source, /task\.status === 'cancelled'/)
assert.match(source, /function isCancelledPublicationTask/)
assert.match(source, /task\.publication_outcome === 'removed' && !hasPublishedIdentity\(task\)/)
assert.match(source, /isCancelled \? copy\.publicationCancelled/)
assert.match(source, /publicationCancelled: 'Публикация отменена'/)
assert.match(source, /function isTerminalPublicationTask/)
assert.match(source, /function isActiveQueueTask/)
assert.match(source, /function isOverduePublicationTask/)
assert.match(source, /function groupPublicationTasks/)
assert.match(source, /const aOverdue = aActive && aTime < now/)
assert.match(source, /\.sort\(\(a, b\) => comparePublicationTasks\(a, b, now\)\)/)
assert.match(source, /taskQueueGroups\.map/)
assert.match(source, /queueOverdue: 'Просроченные активные'/)
assert.match(source, /queueCompleted: 'Опубликованные и завершённые'/)
