...
function WeekCard({ week, isActive = false }: { week: Week, isActive?: boolean }) {
    return (
        <Link to={`/weeks/${week.id}`} className="block group">
            <div className={`relative p-8 rounded-[2.5rem] border transition-all duration-500 overflow-hidden ${
                ...
                <div className="mt-8 flex items-center justify-between">
                    <div className="flex -space-x-2">
                        {[1, 2, 3].map((index) => (
                            <div key={index} className="w-6 h-6 rounded-full border-2 border-white bg-surface-container-high"></div>
                        ))}
                    </div>
...
export default function WeeksList() {
    ...
    const { data: projectData } = useQuery<ApiJson>({
