...
const createPostWorker = () => {
    return new bullmq_1.Worker('postsQueue', async (job) => {
        ...
        const post = await db_1.default.post.findUnique({ where: { id: postId }, include: { week: true } });
        if (!post)
        ...
        try {
            ...
            if (isBatch && post.week_id) {
                const remaining = await db_1.default.post.count({ where: { week_id: post.week_id, status: 'generating' } });
                if (remaining === 0) {
