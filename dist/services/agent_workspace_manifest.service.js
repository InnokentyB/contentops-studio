...
exports.buildAgentWorkspaceManifest = buildAgentWorkspaceManifest;
exports.getAgentWorkspaceUpdate = getAgentWorkspaceUpdate;
exports.loadAgentWorkspaceManifest = loadAgentWorkspaceManifest;
exports.getAgentChatBootstrap = getAgentChatBootstrap;
...
function buildAgentWorkspaceManifest(input) {
    ...
    const unsigned = {
        ...
        configuration_fingerprints: input.settings
            ...
            .map((setting) => ({
            key: setting.key,
            checksum: `sha256:${(0, crypto_1.createHash)('sha256').update(setting.value).digest('hex')}`
        }))
            .sort((a, b) => a.key.localeCompare(b.key))
    };
    const checksum = `sha256:${(0, crypto_1.createHash)('sha256').update(stableJson(unsigned)).digest('hex')}`;
    return { ...unsigned, checksum };
}
function getAgentWorkspaceUpdate(manifest, knownChecksum) {
    if (knownChecksum && knownChecksum === manifest.checksum) {
        return { changed: false, checksum: manifest.checksum, revision: manifest.revision };
    }
    return { changed: true, checksum: manifest.checksum, revision: manifest.revision, manifest };
}
async function loadAgentWorkspaceManifest(projectId, userId) {
    ...
    if (!membership)
        throw new Error('[SECURITY] Project access denied');
...
async function getAgentChatBootstrap(projectId, userId, chatId) {
    const manifest = await loadAgentWorkspaceManifest(projectId, userId);
    const chat = manifest.chats.find((entry) => entry.id === chatId);
    if (!chat)
        throw new Error(`[CHAT_NOT_FOUND] Unknown agent chat '${chatId}'`);
    return {
