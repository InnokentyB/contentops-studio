...
class InitiativeService {
    ...
    requestHash(value) {
        return (0, crypto_1.createHash)('sha256').update(this.stableJson(value)).digest('hex');
    }
    /**
    ...
    async upsertInitiativeWithClient(tx, params) {
    ...
    async linkInitiativesWithClient(tx, params) {
    ...
    async materializePublicationTask(params) {
        return db_1.default.$transaction(async (tx) => {
            ...
            if (params.channelId) {
                ...
                if (!channel)
                    throw new Error(`[INVALID_CHANNEL] Channel ${params.channelId} does not belong to project ${params.projectId}`);
            }
            ...
            const contentData = {
                ...
                quality_report: {
                    ...(existingBridge?.content_item?.quality_report || {}),
    ...
    async syncPublishedPublicationTask(projectId, contentItemId) {
        const result = await db_1.default.initiative.updateMany({
            where: {
                project_id: projectId,
                kind: 'publication',
    ...
    async auditPlanCoverage(params) {
        ...
        const plannerItems = await db_1.default.initiative.findMany({
        ...
        const comparableFields = ['kind', 'due_at', 'start_at', 'end_at', 'decision_at', 'event_at', 'measurement_at'];
        const mismatches = externalItems.flatMap(externalItem => {
            ...
            const fields = comparableFields.filter(field => {
                ...
                if (expected === undefined)
                    return false;
                if (field === 'kind')
                    return plannerItem.kind !== expected;
    ...
    async getOperationalCalendarView(params) {
        ...
        const readinessEntries = await Promise.all(initiativeList.initiatives.map(async (item) => {
            ...
            return [String(item.external_key), readiness];
        }));
        const readinessByKey = Object.fromEntries(readinessEntries);
        const overdueInitiatives = exceptions.overdue_initiatives;
        return {
            ...
            summary: {
                ...
                by_kind: initiativeList.initiatives.reduce((counts, item) => {
