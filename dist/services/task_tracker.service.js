...
exports.taskTrackerService = exports.TaskTrackerService = void 0;
...
class TaskTrackerService {
     ...
     */
    async processOutbox(args) {
        const { projectId, simulateUnreachable, staleOutboxItem } = args;
        if (staleOutboxItem) {
            if (staleOutboxItem.syncVersion < staleOutboxItem.lastSyncedVersion) {
                throw new Error(`[STALE_SYNC_VERSION] Outbox payload version (${staleOutboxItem.syncVersion}) is lower than last synced version (${staleOutboxItem.lastSyncedVersion})`);
            }
        }
        if (simulateUnreachable) {
            await db_1.default.outboxEvent.create({
                data: {
                    project_id: projectId,
            ...
            const pendingCount = await db_1.default.outboxEvent.count({
        ...
        const pendingEvents = await db_1.default.outboxEvent.findMany({
            where: { project_id: projectId, status: 'pending' },
        });
        for (const evt of pendingEvents) {
            await db_1.default.outboxEvent.update({
                where: { id: evt.id },
    ...
    async receiveWebhook(args) {
        ...
        await db_1.default.webhookInbox.create({
            data: {
                project_id: projectId,
                event_id,
                event_type: payload.action || 'webhook.received',
                payload: payload,
                status: 'processed',
            },
        });
     ...
     */
    async reconcileTaskTracker(args) {
...
exports.taskTrackerService = new TaskTrackerService();
exports.default = exports.taskTrackerService;
