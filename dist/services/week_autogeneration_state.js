...
exports.deriveWeekAutomationState = deriveWeekAutomationState;
...
function deriveWeekAutomationState(input) {
    let stage;
    if (!input.themeExists)
        stage = 'awaiting_theme';
    else if (!input.themeAccepted)
        stage = 'awaiting_theme_approval';
    else if (input.topicCount !== 7)
        stage = 'generating_topics';
    else if (input.planDecision === 'rejected')
        stage = 'topics_rejected';
    else if (input.planDecision !== 'approved')
        stage = 'awaiting_topic_approval';
    else if (input.activeWriteCount > 0)
        stage = 'writing';
    else if (input.activeReviewCount > 0)
        stage = 'content_review';
    else if (input.activeVisualCount > 0)
