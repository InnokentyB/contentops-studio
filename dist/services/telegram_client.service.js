...
function assertSafeRemoteImageUrl(rawUrl) {
    ...
    if (parsed.protocol !== 'https:')
        throw new Error('[TELEGRAM_IMAGE_URL_INVALID] Image URL must use HTTPS');
    ...
    if (net.isIP(hostname)) {
        ...
        if (privateAddress)
            throw new Error('[TELEGRAM_IMAGE_URL_FORBIDDEN] Private image hosts are not allowed');
    }
    return parsed;
}
async function loadTelegramRemoteImage(rawUrl, fetchImpl = fetch) {
    ...
    if (!response.ok)
        throw new Error(`[TELEGRAM_IMAGE_FETCH_FAILED] Image server returned ${response.status}`);
    const contentType = response.headers.get('content-type')?.split(';')[0].trim().toLowerCase() || '';
    if (!contentType.startsWith('image/'))
        throw new Error('[TELEGRAM_IMAGE_TYPE_INVALID] Approved asset is not an image');
    const declaredSize = Number(response.headers.get('content-length') || 0);
    if (declaredSize > MAX_TELEGRAM_IMAGE_BYTES)
        throw new Error('[TELEGRAM_IMAGE_TOO_LARGE] Approved asset exceeds 10 MB');
...
class TelegramClientService {
    ...
    async inspectSessionTarget(projectId) {
     ...
     */
    async publishPost(projectId, target, text, imageUrl, scheduleDate, postId, requestHost, options = {}) {
