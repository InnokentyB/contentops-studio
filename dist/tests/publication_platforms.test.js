...
const puppeteer_publisher_service_2 = __importDefault(require("../services/puppeteer_publisher.service"));
...
(0, node_test_1.default)('Dzen cookie parser accepts a copied Cookie header and preserves prefixed cookie rules', () => {
    const cookies = (0, puppeteer_publisher_service_1.parseDzenCookieString)('Cookie: Session_id=abc==; __Host-session=host-value; __Secure-token=secure-value; Path=/; HttpOnly', '.dzen.ru');
    strict_1.default.equal(cookies.length, 3);
    strict_1.default.deepEqual(cookies[0], { name: 'Session_id', value: 'abc==', domain: '.dzen.ru', path: '/' });
    strict_1.default.deepEqual(cookies[1], {
        name: '__Host-session',
        value: 'host-value',
        url: 'https://dzen.ru/',
        path: '/',
    ...
    strict_1.default.deepEqual(cookies[2], {
        name: '__Secure-token',
        value: 'secure-value',
        domain: '.dzen.ru',
        path: '/',
...
(0, node_test_1.default)('Dzen connection supports current channel editor identifiers', () => {
    const publisher = puppeteer_publisher_service_2.default;
    strict_1.default.equal(publisher.dzenChannelEditorUrl({ cookies: 'x=1', channel_id: '6a8029aba055ec36033bf81c' }), 'https://dzen.ru/profile/editor/id/6a8029aba055ec36033bf81c');
    strict_1.default.equal(publisher.dzenChannelEditorUrl({ cookies: 'x=1', channel_url: 'https://dzen.ru/id/6a8029aba055ec36033bf81c' }), 'https://dzen.ru/profile/editor/id/6a8029aba055ec36033bf81c');
});
...
(0, node_test_1.default)('configured Dzen channels prefer connector auto when workflow mode is not explicitly overridden', () => {
    strict_1.default.equal(publication_adapter_service_1.default.prefersAutomaticExecution({ platform: 'dzen', cookies_encrypted: 'enc:v1:test' }), true);
    strict_1.default.equal(publication_adapter_service_1.default.prefersAutomaticExecution({ platform: 'dzen', cookies_encrypted: 'enc:v1:test', workflow_mode: 'approval_required' }), false);
    strict_1.default.equal(publication_adapter_service_1.default.prefersAutomaticExecution({ platform: 'dzen' }), false);
});
